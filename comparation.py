...
freefem_y, freefem_u1 = np.loadtxt("perfil_u1_x05_Re100.txt", unpack=True)
# v ao longo de x no centro horizontal (y = 0.5)
freefem_x, freefem_u2 = np.loadtxt("perfil_u2_y05_Re100.txt", unpack=True)
...
plt.xlabel('u1 (velocidade horizontal)')
plt.ylabel('y')
plt.title('Perfil u1(x=0.5) Re=100')
plt.legend()
plt.grid(True)
plt.tight_layout()
...
plt.xlabel('x')
plt.ylabel('u2 (velocidade vertical)')
plt.title('Perfil u2(y=0.5) Re=100')
plt.legend()
plt.grid(True)
plt.tight_layout()
